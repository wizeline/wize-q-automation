...
test('Ask new question with location', async ({questionsPage,homePage}) => {
  await questionsPage.askQuestion(USERS.STANDAR, DEPARTMENT.ACADEMY,LOCATIONS.GDL)
...
test('Ask new Anonymous question with location', async ({questionsPage,homePage}) => {
  await questionsPage.askQuestion(USERS.ANONYMOUS, DEPARTMENT.ACADEMY,LOCATIONS.GDL)
